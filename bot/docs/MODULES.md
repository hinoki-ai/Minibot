...
7. refresh Team Hunt runtime state
...
13. run Team Hunt suspend or follow actions
...
| --- | --- | --- | --- |
| `route` / `autowalk` | [`lib/bot-core.mjs`](../lib/bot-core.mjs), [`lib/config-store.mjs`](../lib/config-store.mjs), [`lib/route-validation.mjs`](../lib/route-validation.mjs), [`lib/route-packs.mjs`](../lib/route-packs.mjs), [`desktop/renderer.js`](../desktop/renderer.js) | `autowalkEnabled`, `autowalkLoop`, `routeStrictClear`, `routeSpacingEnabled`, `routeFollowExactWaypoints`, `routeRecording`, `routeRecordStep`, `showWaypointOverlay`, `waypoints`, `tileRules`, `waypointRadius`, `walkRepathMs`, `avoidElementalFields`, `avoidFieldCategories`, `cavebotPaused`, `stopAggroHold`, `cavebotName` | Waypoint route execution, route recording, route library persistence, route pack import/export, validation reporting, waypoint overlay, route reset, route resync, ambiguous-crossing recovery, floor-transition relatch/recovery, label-based route loops, helper recovery, corpse return, route spacing leases, route-local snapshots, route action execution, and route-owned bank/shop/NPC/daily-task waypoint dispatch. Route spacing uses live peer positions near the route spine before falling back to stored spacing indices. `routeFollowExactWaypoints` is exact by default, but Team Hunt turns the default off unless a route explicitly sets it; when it is false, plain same-floor walk/node/safe-zone runs may glide to a farther reachable waypoint, while automation, reset/recovery, wait-tile, blocked, floor-changing, or spacing-sensitive segments keep single-step destinations. `routeStrictClear` retries blocked waypoints until clear instead of advancing through blocked-waypoint recovery. |
| `avoidFields` | [`lib/bot-core.mjs`](../lib/bot-core.mjs) | `avoidElementalFields`, `avoidFieldCategories` | Avoids configured field categories while choosing route and combat movement. Categories are `fire`, `energy`, `poison`, `holes`, `stairsLadders`, `teleports`, `traps`, and `invisibleWalls`. Route safety may keep native chase standing only while there is no reachable combat target or no explicit chase-to-kill profile is active. |
| `targeting` | [`lib/bot-core.mjs`](../lib/bot-core.mjs), [`desktop/renderer.js`](../desktop/renderer.js), [`lib/hunt-presets.mjs`](../lib/hunt-presets.mjs) | `monsterNames`, `targetProfiles`, `sharedSpawnMode`, `creatureLedger`, `rangeX`, `rangeY`, `combatRangeX`, `combatRangeY`, `floorTolerance`, `retargetMs` | Hunt queue, per-monster target profiles, shared-spawn policy, creature registry, visible monster/player/NPC ledgers, official hunt presets, target selection, target clearing, transparent target scoring, movement intent reporting, and fallback combat range rules. Current targets or attackable visible targets run the main loop on a fast combat delay, bounded by lower `retargetMs` values, so the next target is picked up without waiting for the normal idle interval. Targeting sends the chosen candidate first plus the selected live candidate batch to the client target set, after route-spacing and shared-spawn filtering. Reachable combat targets in the combat window suspend route movement and allow the configured or profile chase stance to be restored. Same-session peer positions from route/follow coordination count as assist claims, so local sessions can help kill a peer's monster instead of treating it as reserved by shared-spawn or route-spacing policy. Target scoring explains profile order, priority, danger, finish window, HP, threat, reachability, target count, ownership, same-session assist, route role, distance, and current-target stickiness for the top candidates. |
...
| `haste` | [`lib/modules/haste.mjs`](../lib/modules/haste.mjs), [`lib/bot-core.mjs`](../lib/bot-core.mjs), [`desktop/renderer.js`](../desktop/renderer.js) | `hasteEnabled`, `hasteWords`, `hasteHotkey`, `hasteMinMana`, `hasteMinManaPercent`, `hasteCooldownMs`, `hasteRequireNoTargets`, `hasteRequireStationary`, `hasteManaFluidEnabled`, `hasteManaFluidName`, `hasteManaFluidHotkey`, `hasteManaFluidCooldownMs` | Keeps the character hasted from live haste-condition detection. Casts `utani hur` or `utani gran hur`, respects mana and optional target/movement gates, and can drink a configured mana fluid or mana potion from hotbar/open inventory before retrying the haste spell. |
...
| `spellCaster` | [`lib/bot-core.mjs`](../lib/bot-core.mjs) | `spellCasterEnabled`, `spellCasterRules` | Ordered offensive spell and rune rules. Rules gate by MP min, target names, max target distance, target count, cooldown, target requirement, visible-player safety, stationary requirement, and pattern. Patterns are `any`, `adjacent`, `front`, `aligned`, `diagonal`, `pack`, and `aoe`. `front` rules rotate toward the adjacent cardinal target before casting front-tile strike spells. AoE rules explain cast, player-safety, count, cooldown, route tile-rule, safe-tile, ownership, floor, and line-of-sight decisions before emitting either a cast action or rune tile-use action. |
| `distanceKeeper` | [`lib/bot-core.mjs`](../lib/bot-core.mjs), route hunt workspace | `distanceKeeperEnabled`, `distanceKeeperRules`, target-profile distance fields | Kiting and dodge movement. Rules gate by target distance window, monster count, cooldown, beam/wave dodge flags, and target requirement. Behaviors are `retreat`, `kite`, `hold`, and `escape`. Target profiles can also contribute distance behavior. Close melee wave-stance planning is limited to targets already within a short approach window, so a character at range chases directly before trying to path to a diagonal melee stance. |
...
| `antiIdle` | [`lib/bot-core.mjs`](../lib/bot-core.mjs) | `antiIdleEnabled`, `antiIdleIntervalMs` | Idle keepalive. Prefers direct keepalive hooks, then reversible inventory move pulse. Keyboard/input fallback is available only when visible input control is explicitly enabled. Trainer reuses the same pulse timing. |
...
| `partyFollow` | [`lib/bot-core.mjs`](../lib/bot-core.mjs), [`desktop/renderer.js`](../desktop/renderer.js), [`desktop/party-follow-summary.js`](../desktop/party-follow-summary.js) | `teamEnabled`, `partyFollowEnabled`, `partyFollowMembers`, `partyFollowManualPlayers`, `partyFollowMemberRoles`, `partyFollowMemberChaseModes`, `partyFollowDistance`, `partyFollowCombatMode` plus legacy `followChain*` and `teamHuntEnabled` aliases | Runtime key family for Team Hunt roles and route sync. The desktop UI owns these controls from the Team Hunt card/modal; old Follow Chain entry points alias to Team Hunt. Slot 1 leads; each later member follows the name above it. Team Hunt with two or more `partyFollowMembers` activates the ordered follow train even when `partyFollowEnabled` is off. Implicit Team Hunt followers keep route-walking while they have no known leader position, then same-floor shared or remembered leader coordinates can immediately re-path recovery movement instead of waiting through the loose-recovery delay. Same-vocation Team Hunt members publish vocation and current combat target state, and target scoring spreads duplicate vocations across identical visible monsters before assisting a same-vocation peer's current target. Supports multiple independent live chains, live tabs, seen players, manual names, per-member roles and stances, follow-and-fight, follow-only, exact stair recovery, stale same-route hold bypass, same-floor stall recovery with viewport map-click movement before client-path fallback, target clearing for passive roles, route-sync holds through `teamEnabled`, and shared live HP/supply/loot summaries across chain members. |
...
snapshot. They group route, targeting, sustain, loot, refill, banking, alarm
thresholds/actions, party, notes, schema version, and compatibility metadata,
while excluding character-local ledgers, alarm power/sound switches, claims,
active pauses, secrets, and runtime leases.
...
banking without rules, enabled Team Hunt without members, and alarm scopes
...
| [`aoe-solver.mjs`](../lib/modules/aoe-solver.mjs) | `solveAoeAction` | Scores AoE spell and rune casts against target count, player safety, route no-go rules, safe tiles, ownership, floor, and line-of-sight gates. |
...
| [`haste.mjs`](../lib/modules/haste.mjs) | `HASTE_SPELLS`, `DEFAULT_HASTE_WORDS`, `DEFAULT_HASTE_MANA_FLUID_NAME`, `DEFAULT_HASTE_MANA_FLUID_NAMES`, `buildHasteAction` | Builds haste spell and mana-fluid support actions from live haste-condition, mana, target, movement, hotbar, and inventory state. |
...
| [`party-planner.mjs`](../lib/modules/party-planner.mjs) | `buildPartyPlannerSummary` | Builds Team Hunt summary data from live party, member role, route, supply, and loot state. |
...
- [`test/haste.test.mjs`](../test/haste.test.mjs): haste and mana-fluid planner
- [`test/ammo.test.mjs`](../test/ammo.test.mjs): ammo policy, reload, and restock behavior
- [`test/looter.test.mjs`](../test/looter.test.mjs): looting and container routing
- [`test/loot-economics.test.mjs`](../test/loot-economics.test.mjs): loot valuation, capacity decisions, and autosell request planning
