...
| `partyFollow` | [`lib/bot-core.mjs`](../lib/bot-core.mjs), [`desktop/renderer.js`](../desktop/renderer.js), [`desktop/party-follow-summary.js`](../desktop/party-follow-summary.js) | `partyFollowEnabled`, `teamEnabled`, `partyFollowMembers`, `partyFollowManualPlayers`, `partyFollowMemberRoles`, `partyFollowMemberChaseModes`, `partyFollowDistance`, `partyFollowCombatMode` plus legacy `followChain*` and `teamHuntEnabled` aliases | Ordered follow chain and Team Hunt route sync. Slot 1 leads; each later member follows the name above it. Team Hunt with two or more `partyFollowMembers` activates the ordered follow train even when `partyFollowEnabled` is off. Supports multiple independent live chains, live tabs, seen players, manual names, per-member roles and stances, follow-and-fight, follow-only, stair recovery, same-floor stall recovery with viewport map-click movement before client-path fallback, target clearing for passive roles, route-sync holds through `teamEnabled`, and shared live HP/supply/loot summaries across chain members. |
