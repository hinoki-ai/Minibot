...
import {
  ...
  findPage,
  inspectPageIdentity,
  isVisibleInputControlHardDisabled,
  normalizeOptions,
...
test("visible input hard-disable environment values are normalized", () => {
  assert.equal(isVisibleInputControlHardDisabled({ MINIBOT_DISABLE_VISIBLE_INPUT: "1" }), true);
  assert.equal(isVisibleInputControlHardDisabled({ MINIBOT_DISABLE_VISIBLE_INPUT: "true" }), true);
  assert.equal(isVisibleInputControlHardDisabled({ MINIBOT_DISABLE_VISIBLE_INPUT: "0" }), false);
  assert.equal(isVisibleInputControlHardDisabled({ MINIBOT_DISABLE_VISIBLE_INPUT: "off" }), false);
  assert.equal(isVisibleInputControlHardDisabled({}), false);
});
...
test("normalizeOptions defaults the route recorder to 3 SQM and shows waypoint overlays", () => {
  ...
  assert.equal(options.partyFollowLooseRecoveryEnabled, true);
  assert.equal(options.pkAssistEnabled, false);
  assert.equal(options.pkAssistMode, "evade-and-assist");
  assert.deepEqual(options.pkAssistAllies, []);
  assert.equal(options.pkAssistRadiusSqm, 10);
  assert.equal(options.pkAssistRetreatHealthPercent, 45);
  assert.equal(options.pkAssistRetreatDistance, 6);
  assert.equal(options.pkAssistCooldownMs, 650);
...
test("normalizeOptions preserves front-tile spell caster safety gates", () => {
  const options = normalizeOptions({
    spellCasterRules: [
      {
        enabled: true,
        label: "Front rotworm exori flam",
        words: "exori flam",
        targetNames: "Rotworm, Alpha Rotworm",
        minManaPercent: 20,
        maxTargetDistance: 1,
        pattern: "front",
        requireNoPlayers: true,
      },
    ],
  });

  assert.equal(options.spellCasterRules.length, 1);
  assert.equal(options.spellCasterRules[0].pattern, "front");
  assert.deepEqual(options.spellCasterRules[0].targetNames, ["Rotworm", "Alpha Rotworm"]);
  assert.equal(options.spellCasterRules[0].requireNoPlayers, true);
});
...
test("normalizeOptions re-arms elemental field categories when the global field guard is enabled", () => {
  const options = normalizeOptions({
...
test("normalizeOptions normalizes PK assist module settings", () => {
  const options = normalizeOptions({
    ...
    pkAssistMode: "help",
    pkAssistAllies: "Knight Alpha\nScout Beta",
    pkAssistRadiusSqm: "14",
    pkAssistRetreatHealthPercent: 37.8,
    pkAssistRetreatDistance: "7",
    pkAssistCooldownMs: "900",
  });

  assert.equal(options.pkAssistEnabled, true);
...
test("chooseFollowTrainAction can disable loose same-floor recovery walks", () => {
  const bot = new MinibiaTargetBot({
    partyFollowEnabled: true,
    partyFollowMembers: ["Knight Alpha", "Scout Beta"],
    partyFollowDistance: 1,
    partyFollowLooseRecoveryEnabled: false,
  });
  bot.getNow = () => 5_000;
  bot.followTrainState = {
    leaderName: "Knight Alpha",
...
test("chooseFollowTrainAction clicks a reachable stair from two tiles away after native follow stalls", () => {
  const bot = new MinibiaTargetBot({
    partyFollowEnabled: true,
    partyFollowMembers: ["Knight Alpha", "Scout Beta"],
    partyFollowDistance: 1,
  ...
  bot.getNow = () => 10_000;
  bot.followTrainState = bot.buildFollowTrainState({
    leaderName: "Knight Alpha",
    leaderKey: "knight alpha",
    currentState: "FOLLOWING",
    lastSeenAt: 9_900,
    lastSeenPosition: { x: 100, y: 100, z: 7 },
    lastLeaderSource: "visible",
    activeFollowTargetId: 10,
    activeFollowTargetKey: "knight alpha",
    lastFollowAttemptAt: 8_000,
    lastRecoveryWalkAt: 8_000,
    lastStairAttemptAt: 8_000,
  });
  ...
  const action = bot.chooseFollowTrainAction({
    ...
    isMoving: false,
    pathfinderAutoWalking: true,
    pathfinderFinalDestination: { x: 101, y: 99, z: 8 },
...
test("chooseFollowTrainAction refuses projected floor recovery when stair markers are missing", () => {
  const bot = new MinibiaTargetBot({
    partyFollowEnabled: true,
    partyFollowMembers: ["Knight Alpha", "Scout Beta"],
    partyFollowDistance: 1,
...
test("chooseFollowTrainAction uses route stairs before projected cross-floor coordinates", () => {
  const bot = new MinibiaTargetBot({
    partyFollowEnabled: true,
    partyFollowMembers: ["Czarnobrat", "Zlocimir Wielkoportf"],
    partyFollowDistance: 1,
    waypoints: [
      { x: 32811, y: 32161, z: 9, type: "stairs-up", label: "Waypoint 024" },
      { x: 32810, y: 32155, z: 8, type: "node", label: "Waypoint 025", radius: 1 },
      { x: 32816, y: 32144, z: 8, type: "stairs-up", label: "Waypoint 028" },
      { x: 32821, y: 32144, z: 7, type: "node", label: "Waypoint 029", radius: 1 },
    ],
  });
  bot.getNow = () => 10_000;
  bot.routeIndex = 0;
  bot.followTrainCoordinationState = {
    selfInstanceId: "zlocimir-wielkoportf",
    members: [
      {
        instanceId: "czarnobrat",
        characterName: "Czarnobrat",
        playerPosition: { x: 32822, y: 32145, z: 7 },
        updatedAt: 10_000,
      },
      {
        instanceId: "zlocimir-wielkoportf",
        characterName: "Zlocimir Wielkoportf",
        playerPosition: { x: 32812, y: 32161, z: 9 },
        updatedAt: 10_000,
      },
    ],
  };
  ...
  assert.equal(action?.kind, "follow-train-walk");
  assert.deepEqual(action?.destination, { x: 32811, y: 32161, z: 9 });
  assert.equal(action?.progressKey, "32811,32161,9");
  assert.equal(action?.reason, "route-floor-change");
  assert.equal(action?.targetName, "Czarnobrat");
});
...
test("refreshFollowTrainRuntime passively syncs follower route progress while fighting", () => {
  const bot = new MinibiaTargetBot({
    autowalkEnabled: true,
    partyFollowEnabled: true,
    partyFollowMembers: ["Knight Alpha", "Scout Beta"],
...
test("chooseRouteAction holds the follow-chain pilot while a downstream member is fighting", () => {
  const bot = new MinibiaTargetBot({
    autowalkEnabled: true,
    partyFollowEnabled: true,
    partyFollowMembers: ["Knight Alpha", "Scout Beta"],
  ...
  assert.equal(action?.reason, "follow-chain-combat");
  assert.equal(bot.routeIndex, 0);
});

test("chooseRouteAction clears a floor landing before holding for downstream combat", () => {
  const bot = new MinibiaTargetBot({
    autowalkEnabled: true,
    partyFollowEnabled: true,
    partyFollowMembers: ["Knight Alpha", "Scout Beta", "Druid Gamma", "Mule Delta"],
    waypoints: [
      { x: 100, y: 100, z: 7, type: "stairs-down" },
      { x: 102, y: 100, z: 8, type: "node", radius: 0 },
      { x: 104, y: 100, z: 8, type: "walk" },
    ],
  });
  bot.routeIndex = 1;
  bot.lastFollowTrainFloorTransition = {
    fromPosition: { x: 100, y: 100, z: 7 },
    toPosition: { x: 100, y: 100, z: 8 },
    accessPosition: { x: 100, y: 100, z: 7 },
    path: [{ x: 100, y: 100, z: 7 }],
    direction: "down",
    at: 10_000,
  };
  bot.getNow = () => 10_100;
  ...
  assert.equal(action?.kind, "walk");
  assert.equal(action?.destination?.x, 104);
  assert.equal(action?.destination?.y, 100);
  assert.equal(action?.destination?.z, 8);
  assert.equal(bot.routeIndex, 2);
});

test("chooseRouteAction stops after follow-chain landing clearance while followers change floors", () => {
  const bot = new MinibiaTargetBot({
    autowalkEnabled: true,
    partyFollowEnabled: true,
    partyFollowMembers: ["Knight Alpha", "Scout Beta", "Druid Gamma", "Mule Delta"],
  ...
  const action = bot.chooseRouteAction({
    ...
    isMoving: true,
    pathfinderAutoWalking: true,
    pathfinderFinalDestination: { x: 110, y: 100, z: 8 },
  });

  assert.equal(action?.kind, "hold-route");
  assert.equal(action?.reason, "follow-chain-floor-after");
  assert.equal(action?.interrupt, true);
  assert.equal(bot.routeIndex, 2);
});

test("chooseRouteAction holds a Team Hunt leader before changing floors until followers are close", () => {
  const bot = new MinibiaTargetBot({
    autowalkEnabled: true,
    teamEnabled: true,
    partyFollowMembers: ["Knight Alpha", "Scout Beta"],
    ...
    waypoints: [
      ...
      { x: 100, y: 101, z: 9, type: "node", radius: 1 },
    ],
  });
  bot.followTrainCoordinationState = {
    selfInstanceId: "leader",
    members: [
      ...
      },
      {
        ...
        characterName: "Scout Beta",
        enabled: true,
        playerPosition: { x: 100, y: 105, z: 8 },
      },
    ],
  };
  ...
  assert.equal(action?.kind, "hold-route");
  assert.equal(action?.reason, "team-hunt-sync");
  assert.equal(bot.routeIndex, 0);
});

test("chooseRouteAction holds a Team Hunt leader after changing floors until followers arrive", () => {
  const bot = new MinibiaTargetBot({
    autowalkEnabled: true,
    teamEnabled: true,
    partyFollowMembers: ["Knight Alpha", "Scout Beta"],
    ...
    waypoints: [
      { x: 100, y: 100, z: 8, type: "stairs-down" },
      { x: 100, y: 101, z: 9, type: "node", radius: 1 },
      { x: 105, y: 101, z: 9, type: "node", radius: 1 },
    ],
  });
  bot.routeIndex = 1;
  bot.followTrainCoordinationState = {
    ...
    members: [
      {
        ...
        characterName: "Knight Alpha",
        enabled: true,
        playerPosition: { x: 100, y: 101, z: 9 },
      },
      {
        instanceId: "follower",
        characterName: "Scout Beta",
...
test("castAttackSpell turns toward front-tile spells before casting words", async () => {
  const bot = new MinibiaTargetBot({ inputControlEnabled: true });
  const calls = [];

  bot.executeKeyboardTurn = async (direction) => {
    calls.push(`turn:${direction}`);
    return { ok: true, transport: "keyboard-turn", direction };
  };
  bot.castWords = async (action) => {
    calls.push(`cast:${action.words}`);
    return { ok: true, words: action.words };
  };

  const result = await bot.castAttackSpell({
    type: "attack-spell",
    words: "exori flam",
    turnDirection: "north",
  });
  ...
  assert.equal(result?.turn?.transport, "keyboard-turn");
  assert.deepEqual(calls, ["turn:north", "cast:exori flam"]);
});
...
test("chooseTarget assists monsters engaged by another local desktop session", () => {
  const bot = new MinibiaTargetBot({
    autowalkEnabled: true,
    monsterNames: ["Dragon"],
    sharedSpawnMode: "respect-others",
  ...
  bot.setRuntimeLoad({
    ...
    sessionIndex: 0,
    active: true,
    localPlayerNames: ["Own Knight", "Czarnobrat"],
  });
  ...
  const snapshot = {
    ...
    playerPosition: { x: 100, y: 100, z: 8 },
    currentTarget: null,
    visiblePlayers: [localPlayer],
    visibleCreatures: [sharedTarget],
    candidates: [sharedTarget],
  };

  const selection = bot.chooseTarget(snapshot);

  assert.equal(bot.isEntryBlockedBySharedSpawn(sharedTarget, snapshot), false);
  assert.equal(selection?.chosen?.id, 10);
});
...
test("disconnect and death alarms do not pause route unless protector controls are enabled", () => {
  const bot = new MinibiaTargetBot({
    autowalkEnabled: true,
    cavebotPaused: false,
    stopAggroHold: false,
  ...
  const disconnectStatus = bot.refreshProtectorStatus({
    ready: false,
    reason: "disconnected",
    connection: { connected: false },
  }, { allowActions: true, now: 5_500 });

  assert.equal(disconnectStatus.active, true);
  assert.equal(disconnectStatus.highestSeverity, "critical");
  assert.equal(disconnectStatus.actions.includes("sound"), true);
  assert.equal(disconnectStatus.actions.includes("pause-route"), false);
  assert.equal(disconnectStatus.actions.includes("stop-targeter"), false);
  assert.deepEqual(disconnectStatus.pausedModules, []);
  assert.equal(bot.options.cavebotPaused, false);
  assert.equal(bot.options.stopAggroHold, false);

  const deathStatus = bot.refreshProtectorStatus({
    ready: true,
    reason: "dead",
    connection: { playerIsDead: true },
  }, { allowActions: true, now: 6_000 });

  assert.equal(deathStatus.active, true);
  assert.equal(deathStatus.actions.includes("pause-route"), false);
  assert.equal(deathStatus.actions.includes("stop-targeter"), false);
  assert.deepEqual(deathStatus.pausedModules, []);
  assert.equal(bot.options.cavebotPaused, false);
  assert.equal(bot.options.stopAggroHold, false);
  assert.equal(emitted.some((event) => event.type === "protector-paused"), false);
});

test("disconnect alarms honor explicit protector pause controls", () => {
  const bot = new MinibiaTargetBot({
    autowalkEnabled: true,
    alarmsProtectorEnabled: true,
    alarmsPauseRoute: true,
  ...
  const status = bot.refreshProtectorStatus({
    ready: false,
    reason: "disconnected",
    connection: { connected: false },
  }, { allowActions: true, now: 6_500 });
...
test("protector player alarms ignore local live session characters", () => {
  const bot = new MinibiaTargetBot({
    alarmsProtectorEnabled: true,
  ...
  assert.deepEqual(status.activeAlarms, []);
});

test("protector player alarms still fire for strangers after local sessions are filtered", () => {
  const bot = new MinibiaTargetBot({
    alarmsProtectorEnabled: true,
    alarmsPlayerEnabled: true,
    alarmsPlayerRadiusSqm: 8,
  ...
  assert.equal(status.active, true);
  assert.equal(status.activeAlarms[0]?.type, "nearby-player");
  assert.equal(status.activeAlarms[0]?.target?.name, "PK Two");
});
...
test("chooseTarget keeps a sticky current target instead of switching to a fresh blocker", () => {
  const bot = new MinibiaTargetBot({
    monsterNames: ["Dragon"],
    targetProfiles: [
      {
  ...
  const selection = bot.chooseTarget({
    ...
    playerPosition: { x: 100, y: 100, z: 8 },
    currentTarget: currentDragon,
    visibleCreatures: [freshDragon, currentDragon],
    candidates: [freshDragon, currentDragon],
  });
...
test("chooseTarget keeps a chase-to-kill current target outside the combat window", () => {
  const bot = new MinibiaTargetBot({
    monsterNames: ["Dragon"],
    rangeX: 7,
    rangeY: 5,
...
test("shouldHoldRouteForCombat holds chase-to-kill current targets outside the combat window", () => {
  ...
  const snapshot = createModuleSnapshot({
    playerPosition: { x: 100, y: 100, z: 8 },
    currentTarget: currentDragon,
    visibleCreatures: [currentDragon],
    candidates: [],
  });
...
test("chooseSpellCaster gates front-tile rotworm spells away from visible players", () => {
  const bot = new MinibiaTargetBot({
  ...
  const baseSnapshot = createModuleSnapshot({
    playerPosition: { x: 100, y: 100, z: 8 },
    currentTarget: rotworm,
    visibleCreatures: [rotworm],
    candidates: [rotworm],
  });

  const action = bot.chooseSpellCaster(baseSnapshot);
  assert.equal(action?.type, "attack-spell");
  assert.equal(action?.words, "exori flam");
  assert.equal(action?.turnDirection, "east");

  assert.equal(bot.chooseSpellCaster({
    ...baseSnapshot,
    visiblePlayers: [
      { id: 20, name: "Knight Alpha", position: { x: 102, y: 100, z: 8 } },
    ],
  ...
  assert.equal(bot.chooseSpellCaster({
    ...baseSnapshot,
    currentTarget: { ...rotworm, id: 11, name: "Snake" },
    visibleCreatures: [{ ...rotworm, id: 11, name: "Snake" }],
    candidates: [{ ...rotworm, id: 11, name: "Snake" }],
  }), null);
});
...
test("mana trainer keeps explicit words instead of falling back to light", () => {
  const bot = new MinibiaTargetBot({
    autoLightEnabled: false,
    autoLightRules: [
      {
  ...
  assert.equal(bot.chooseLight(snapshot, vocationProfile), null);
  assert.equal(bot.chooseManaTrainer(snapshot, vocationProfile)?.words, "adori vita vis");
});
...
test("chooseDistanceKeeper interrupts active team-hunt movement for dragon beam dodges", () => {
  const bot = new MinibiaTargetBot({
    teamEnabled: true,
    partyFollowMembers: ["Knight Alpha", "Mage Beta"],
    partyFollowCombatMode: "follow-and-fight",
  ...
  assert.equal(bot.isFollowTrainAttackAndFollow({ ready: true, playerName: "Mage Beta" }), true);
  assert.equal(action?.type, "distance-keeper");
  assert.equal(action?.reason, "dodge");
  assert.equal(action?.interruptMovement, true);
  assert.equal(action?.destination?.x, 100);
  assert.notEqual(action?.destination?.y, 100);
});

test("chooseDistanceKeeper does not interrupt movement already heading to a safer dragon dodge tile", () => {
  const bot = new MinibiaTargetBot({
    teamEnabled: true,
    partyFollowMembers: ["Knight Alpha", "Mage Beta"],
    partyFollowCombatMode: "follow-and-fight",
  ...
  const action = bot.chooseDistanceKeeper({
    ...
    pathfinderAutoWalking: true,
    autoWalkStepsRemaining: 1,
    pathfinderFinalDestination: { x: 100, y: 101, z: 8 },
  });
...
test("chooseDistanceKeeper keeps approaching a sticky aggressive current target outside the target box", () => {
  const bot = new MinibiaTargetBot({
    distanceKeeperEnabled: false,
    monsterNames: ["Dragon"],
    targetProfiles: [
  ...
  assert.equal(bot.getFocusTarget(snapshot)?.id, 10);
  assert.equal(action?.type, "distance-keeper");
  assert.equal(action?.destination?.x, 101);
  assert.equal(action?.destination?.z, 8);
  assert.equal(action?.nextDistance, 7);
  assert.equal(action?.reason, "approach");
});

test("chooseDistanceKeeper chases directly before planning close melee wave stance", () => {
  const bot = new MinibiaTargetBot({
    distanceKeeperEnabled: false,
    targetProfiles: [
      {
  ...
  assert.equal(action?.plannedStance, null);
  assert.equal(action?.reason, "approach");
});
...
test("chooseDistanceKeeper allows the immediate floor-transition ring during combat", () => {
  for (const hazard of [
    { categories: ["holes"], labels: ["Hole"] },
    { categories: ["stairsLadders"], labels: ["Stairs"] },
  ]) {
    const bot = new MinibiaTargetBot({
      avoidElementalFields: false,
      avoidFieldCategories: {
    ...
    assert.equal(action?.type, "distance-keeper", hazard.labels[0]);
    assert.deepEqual(action?.destination, { x: 100, y: 99, z: 8 }, hazard.labels[0]);
    assert.equal(action?.reason, "dodge", hazard.labels[0]);
  }
});
...
test("chooseDistanceKeeper keeps route floor-transition waypoints unsafe during spacing holds", () => {
  for (const type of ["stairs-down", "shovel-hole"]) {
    const bot = new MinibiaTargetBot({
      autowalkEnabled: true,
      autowalkLoop: true,
    ...
    bot.getRouteSpacingAdvice = () => ({ hold: true });
    ...
    assert.equal(bot.shouldHoldRouteForCombat(snapshot), true, type);
    assert.equal(bot.shouldGuardCombatFloorTransitions(snapshot), true, type);
...
test("chooseRouteAction replans stale zero-step client autowalk instead of waiting", () => {
  const bot = new MinibiaTargetBot({
    autowalkEnabled: true,
    autowalkLoop: false,
    waypointRadius: 0,
  ...
  assert.equal(action?.kind, "walk");
  assert.equal(action?.destination?.x, 5);
  assert.equal(action?.progressKey, "5,0,8");
});
...
test("chooseRouteAction can route away from floor transition danger zones outside combat", () => {
  ...
  assert.equal(bot.getDistanceKeeperTileMap(snapshot).has("11,10,8"), true);
...
test("chooseRouteAction glides to the farthest safe reachable waypoint on a plain route segment", () => {
  ...
  assert.equal(bot.options.routeFollowExactWaypoints, false);
  bot.resetRoute(0);
  const action = bot.chooseRouteAction({
    ready: true,
    playerPosition: { x: 100, y: 100, z: 8 },
...
test("chooseRouteAction does not glide past recorded waypoints in exact mode", () => {
  const bot = new MinibiaTargetBot({
    autowalkEnabled: true,
    autowalkLoop: false,
    waypointRadius: 0,
...
test("resyncRouteProgress relatches non-exact route state to a recently accepted glide destination", () => {
  const bot = new MinibiaTargetBot({
    autowalkEnabled: true,
    autowalkLoop: false,
    routeFollowExactWaypoints: false,
...
test("resyncRouteProgress ignores stale glide targets in exact mode", () => {
  const bot = new MinibiaTargetBot({
    autowalkEnabled: true,
    autowalkLoop: false,
    waypointRadius: 0,
...
test("chooseRouteAction does not rewalk intermediate waypoints during an active glide", () => {
  const bot = new MinibiaTargetBot({
    autowalkEnabled: true,
    autowalkLoop: false,
    routeFollowExactWaypoints: false,
  ...
  try {
    ...
    const action = bot.chooseRouteAction({
      ...
      isMoving: true,
      pathfinderAutoWalking: true,
      pathfinderFinalDestination: { x: 105, y: 100, z: 8 },
    });

    assert.equal(action, null);
    assert.equal(bot.routeIndex, 1);
    assert.equal(bot.lastConfirmedWaypointIndex, 0);
  } finally {
    Date.now = realDateNow;
  }
});

test("chooseRouteAction relatches to the glide target after skipped intermediate snapshots", () => {
  const bot = new MinibiaTargetBot({
    autowalkEnabled: true,
    autowalkLoop: false,
    routeFollowExactWaypoints: false,
  ...
  try {
    ...
    assert.equal(action, null);
    assert.equal(bot.routeIndex, 4);
    assert.equal(bot.routeComplete, true);
    assert.equal(bot.lastConfirmedWaypointIndex, 4);
  } finally {
    Date.now = realDateNow;
  }
});

test("chooseRouteAction treats near intermediate waypoints as passed during recent glide progress", () => {
  const bot = new MinibiaTargetBot({
    autowalkEnabled: true,
    autowalkLoop: false,
    routeFollowExactWaypoints: false,
  ...
  bot.getNow = () => now;
  bot.resetRoute(0);
  bot.setLastWalkAttempt("102,100,8", { x: 100, y: 100, z: 8 }, {
    destination: bot.options.waypoints[2],
    glideTargetIndex: 4,
    now,
    pendingProgress: true,
  });
  ...
  try {
    ...
    const action = bot.chooseRouteAction({
      ready: true,
      playerPosition: { x: 106, y: 101, z: 8 },
      currentTarget: null,
      candidates: [],
      visibleCreatures: [],
      isMoving: false,
    ...
    assert.equal(action?.kind, "walk");
    assert.notDeepEqual(action?.destination, bot.options.waypoints[0]);
    assert.notDeepEqual(action?.destination, bot.options.waypoints[1]);
    assert.notDeepEqual(action?.destination, bot.options.waypoints[2]);
  } finally {
    Date.now = realDateNow;
  }
});
...
test("resyncRouteProgress keeps the Venore exit spine instead of relatching onto the nearby entry branch", () => {
  const waypoints = Array.from({ length: 35 }, (_entry, index) => ({
    x: 1000 + (index * 10),
    y: 1000,
    z: 6,
    type: "walk",
  }));
  waypoints[3] = { x: 32823, y: 32145, z: 6, type: "node", radius: 1 };
  waypoints[30] = { x: 32820, y: 32145, z: 6, type: "stand", radius: 1 };
  waypoints[31] = { x: 32817, y: 32144, z: 6, type: "stairs-up" };
  ...
  const resynced = bot.resyncRouteProgress({
    ready: true,
    playerPosition: { x: 32824, y: 32145, z: 6 },
    currentTarget: null,
    candidates: [],
    visibleCreatures: [],
    isMoving: false,
  ...
  assert.equal(resynced, false);
  assert.equal(bot.routeIndex, 30);
});
...
test("node waypoints default to a 5x5 checkpoint area unless overridden", () => {
  const bot = new MinibiaTargetBot({
    autowalkEnabled: true,
    autowalkLoop: false,
    waypointRadius: 0,
    waypoints: [
      { x: 100, y: 100, z: 8, type: "node" },
      { x: 108, y: 100, z: 8, type: "walk" },
    ],
  });
  ...
  assert.equal(bot.getWaypointRadius(bot.options.waypoints[0]), 2);
  assert.equal(bot.lastConfirmedWaypointIndex, 0);
  assert.equal(bot.routeIndex, 1);
  assert.equal(action?.kind, "walk");
  assert.deepEqual(action?.waypoint, bot.options.waypoints[1]);

  const overrideBot = new MinibiaTargetBot({
    autowalkEnabled: true,
    autowalkLoop: false,
    waypointRadius: 0,
    waypoints: [
  ...
  overrideBot.resetRoute(0);

  const overrideAction = overrideBot.chooseRouteAction({
    ready: true,
    playerPosition: { x: 102, y: 100, z: 8 },
    currentTarget: null,
    candidates: [],
  ...
  assert.equal(overrideBot.getWaypointRadius(overrideBot.options.waypoints[0]), 1);
  assert.equal(overrideBot.lastConfirmedWaypointIndex, null);
  assert.equal(overrideBot.routeIndex, 0);
  assert.equal(overrideAction?.kind, "walk");
  assert.deepEqual(overrideAction?.waypoint, overrideBot.options.waypoints[0]);
});

test("exact walk waypoints ignore configured reach radius", () => {
  const bot = new MinibiaTargetBot({
    autowalkEnabled: true,
    autowalkLoop: false,
    waypointRadius: 1,
    waypoints: [
      { x: 100, y: 100, z: 8, type: "walk", radius: 1 },
      { x: 101, y: 100, z: 8, type: "walk" },
    ],
  });

  assert.equal(bot.options.routeFollowExactWaypoints, true);
  assert.equal(bot.getWaypointRadius(bot.options.waypoints[0]), 0);
  assert.equal(bot.isWaypointReached({ x: 99, y: 100, z: 8 }, bot.options.waypoints[0]), false);
...
test("chooseRouteAction requires exact floor-transition sqms even with waypoint radius", () => {
  for (const type of ["stairs-up", "stairs-down", "ladder", "rope", "shovel-hole", "exani-tera"]) {
    const bot = new MinibiaTargetBot({
      autowalkEnabled: true,
      autowalkLoop: false,
      routeFollowExactWaypoints: false,
      waypointRadius: 1,
      waypoints: [
        { x: 100, y: 100, z: 8, type, radius: 2 },
        { x: 100, y: 100, z: type === "stairs-up" || type === "rope" || type === "exani-tera" ? 7 : 9, type: "walk" },
      ],
    });

    bot.resetRoute(0);
    const playerPosition = { x: 99, y: 100, z: 8 };
    const snapshot = {
      ready: true,
      playerPosition,
      currentTarget: null,
    ...
    assert.equal(bot.getWaypointRadius(bot.options.waypoints[0]), 0, type);
    assert.equal(bot.isWaypointReached(playerPosition, bot.options.waypoints[0]), false, type);

    const action = bot.chooseRouteAction(snapshot);

    assert.equal(action?.kind, "walk", type);
    assert.deepEqual(action?.destination, bot.options.waypoints[0], type);
    assert.equal(bot.routeIndex, 0, type);
  }
});

test("resyncRouteProgress does not skip a mandatory stair while still on its source floor", () => {
  const bot = new MinibiaTargetBot({
    autowalkEnabled: true,
    autowalkLoop: false,
    waypointRadius: 0,
  ...
  assert.equal(bot.resyncRouteProgress(snapshot, { forceFullRouteSweep: true }), false);
  assert.equal(bot.routeIndex, 0);
...
test("chooseNoGoZoneEscape steps off a visible stair tile during combat", () => {
  ...
  assert.equal(action?.allowFloorTransitionDangerZoneStep, true);
});
...
test("chooseRouteAction pauses a follower before it walks into a close predecessor spacing window", () => {
...
test("chooseRouteAction lets one tied session open a loose split and parks the other", () => {
  ...
  const early = new MinibiaTargetBot({
    autowalkEnabled: true,
    autowalkLoop: true,
    waypoints,
  });
  early.routeCoordinationState = {
    selfInstanceId: "early",
    members: baseMembers,
  };

  const earlyAction = early.chooseRouteAction(snapshot);
  assert.equal(earlyAction?.kind, "walk");
  assert.equal(earlyAction?.destination?.x, 101);
  assert.equal(early.routeSpacingHold, null);

  const late = new MinibiaTargetBot({
    autowalkEnabled: true,
    autowalkLoop: true,
    waypoints,
  });
  late.routeCoordinationState = {
    selfInstanceId: "late",
    members: baseMembers,
  };

  assert.equal(late.chooseRouteAction(snapshot), null);
...
test("Team Hunt route sync bypasses a stale stuck follower instead of holding forever", () => {
  const waypoints = [
    { x: 100, y: 200, z: 8, type: "walk" },
    { x: 101, y: 200, z: 8, type: "walk" },
    { x: 102, y: 200, z: 8, type: "walk" },
  ...
  bot.routeCoordinationState = {
    ...
    members: [
      ...
      },
      {
        ...
        routeIndex: 0,
        confirmedIndex: 0,
        playerPosition: { x: 100, y: 200, z: 4 },
        startedAt: 2,
      },
    ],
  };
  ...
  assert.equal(walkAction?.kind, "walk");
  assert.equal(walkAction?.destination?.x, 102);
  assert.equal(walkAction?.destination?.y, 200);
  assert.equal(walkAction?.destination?.z, 8);
  assert.equal(bot.teamHuntHold, null);
  assert.equal(bot.teamHuntBypass?.peerInstanceId, "scout");
});
...
test("chooseTarget can assist a same-session monster beyond the normal route spacing lane", () => {
  const waypoints = Array.from({ length: 100 }, (_, index) => ({
    x: 100 + index,
    y: 200,
    z: 8,
  ...
  bot.setRuntimeLoad({
    ...
    sessionIndex: 0,
    active: true,
    localPlayerNames: ["Follower", "Leader"],
  });
  bot.routeCoordinationState = {
    selfInstanceId: "follower",
    members: [
...
test("chooseTarget keeps local combat available while route spacing is collapsed", () => {
  ...
  assert.equal(selection?.chosen?.id, 77);
  assert.equal(bot.routeSpacingHold, null);
  assert.equal(bot.lastTargetScoreReport?.candidates[0]?.selected, true);
});

test("chooseRouteAction bounces backward on visible route tiles during a cramped spacing hold", () => {
  const waypoints = Array.from({ length: 40 }, (_, index) => ({
    x: 100 + index,
    y: 200,
    z: 8,
    type: "walk",
  ...
  const action = bot.chooseRouteAction(snapshot);
  assert.equal(action?.kind, "walk");
  assert.equal(action?.walkReason, "route-spacing-bounce");
  assert.equal(action?.destination?.x, 108);
  assert.equal(action?.destination?.y, 200);
  assert.equal(action?.destination?.z, 8);
  assert.equal(bot.routeIndex, 10);
  assert.equal(bot.routeSpacingHold?.activeBounceDestinationKey, "108,200,8");

  now += 250;
  const movingAction = bot.chooseRouteAction({
    ...snapshot,
    isMoving: true,
    pathfinderAutoWalking: true,
    pathfinderFinalDestination: action.destination,
  });
  assert.equal(movingAction, null);
});
...
test("chooseRouteAction lets active combat take priority over spacing collapse", () => {
  const waypoints = Array.from({ length: 72 }, (_, index) => ({
    x: 33000 + index,
    y: 32600,
    z: 8,
  ...
  const snapshot = {
    ready: true,
    playerPosition: { x: 33033, y: 32600, z: 8 },
    currentTarget: {
      id: 77,
      name: "Dragon",
      position: { x: 33033, y: 32600, z: 8 },
      reachableForCombat: true,
      withinCombatBox: true,
      withinCombatWindow: true,
    },
...
test("chooseRouteAction keeps a long stride until a floor-transition danger zone is near", () => {
  const bot = new MinibiaTargetBot({
    autowalkEnabled: true,
    waypoints: [
      { x: 108, y: 100, z: 8, type: "walk" },
  ...
  assert.equal(action?.kind, "walk");
  assert.deepEqual(action?.destination, { x: 103, y: 100, z: 8 });
  assert.equal(action?.progressKey, "108,100,8");
});

test("chooseRouteAction steps around a hole danger-zone corner instead of bouncing backward", () => {
  const bot = new MinibiaTargetBot({
    autowalkEnabled: true,
    waypointRadius: 0,
    waypoints: [
      { x: 32022, y: 32211, z: 8, type: "walk" },
    ],
  });
  const reachableWalkableTiles = [
    [32021, 32207], [32021, 32208], [32021, 32209],
    [32022, 32206], [32022, 32207], [32022, 32208], [32022, 32209], [32022, 32210], [32022, 32211],
    [32023, 32206], [32023, 32207], [32023, 32208], [32023, 32209], [32023, 32210], [32023, 32211],
    [32024, 32206], [32024, 32207], [32024, 32208], [32024, 32209], [32024, 32210], [32024, 32211],
    [32025, 32207], [32025, 32208], [32025, 32209], [32025, 32210],
  ].map(([x, y]) => ({ x, y, z: 8 }));

  const action = bot.chooseRouteAction({
    ready: true,
    playerPosition: { x: 32025, y: 32209, z: 8 },
    currentTarget: null,
    visibleCreatures: [],
    visiblePlayers: [],
    candidates: [],
    hazardTiles: [
      {
        position: { x: 32023, y: 32209, z: 8 },
        categories: ["holes", "stairsLadders"],
        labels: ["hole"],
      },
    ],
    walkableTiles: reachableWalkableTiles,
    reachableWalkableTiles,
    safeTiles: reachableWalkableTiles,
    reachableTiles: reachableWalkableTiles,
    isMoving: false,
    pathfinderAutoWalking: false,
    pathfinderFinalDestination: null,
  });
  ...
  assert.equal(action?.kind, "walk");
  assert.deepEqual(action?.destination, { x: 32025, y: 32210, z: 8 });
  assert.equal(action?.progressKey, "32022,32211,8");
});
...
test("target sends the focused creature first with the selected live target batch", async () => {
  ...
  const freshDragon = {
    id: 11,
    name: "Dragon",
    __position: { x: 102, y: 100, z: 8 },
  };
  const targetSetSizes = [];
      ...
      return Function("window", "globalThis", `return ${expression};`)({
        gameClient: {
          ...
          world: {
            ...
            targetMonster(targets) {
              const list = [...targets];
              targetSetSizes.push(list.length);
              player.__target = list[0] || null;
            },
          },
        },
      }, {});
...
test("restorePreferredChaseMode keeps aggressive chase on safe hunt nodes", async () => {
  const bot = new MinibiaTargetBot({
    autowalkEnabled: true,
    autowalkLoop: true,
    chaseMode: "aggressive",
    waypoints: [
      { x: 95, y: 100, z: 8, type: "stairs-down" },
      { x: 98, y: 100, z: 9, type: "stairs-up" },
      { x: 100, y: 100, z: 8, type: "node", radius: 2 },
      { x: 106, y: 100, z: 8, type: "node", radius: 2 },
      { x: 120, y: 100, z: 8, type: "stairs-down" },
    ],
  });
  const selector = {
    currentChaseMode: 0,
...
test("restorePreferredChaseMode keeps aggressive chase for chase-to-kill targets outside the combat window", async () => {
  const bot = new MinibiaTargetBot({
    autowalkEnabled: true,
    autowalkLoop: true,
    chaseMode: "aggressive",
...
test("restorePreferredChaseMode keeps native chase standing during wave dodges near stairs", async () => {
  const bot = new MinibiaTargetBot({
    autowalkEnabled: true,
    autowalkLoop: true,
    chaseMode: "aggressive",
...
test("walk steps by keyboard before viewport fallback when standing in an elemental field", async () => {
  const bot = new MinibiaTargetBot();
  const destination = { x: 100, y: 101, z: 8 };
  const events = [];
  let clickCalls = 0;
  ...
  assert.equal(events.find((event) => event.type === "walk")?.payload?.fallbackReason, "native pathfinder unavailable");
});

test("walk tries a repeated one-sqm keyboard step before another viewport click", async () => {
...
test("reposition allows combat floor-transition ring destinations before clicking", async () => {
  const destination = { x: 100, y: 99, z: 8 };
  const bot = new MinibiaTargetBot({
    autowalkEnabled: true,
    autowalkLoop: true,
...
test("executeKeyboardTurn sends a control-arrow turn pulse", async () => {
  const bot = new MinibiaTargetBot({ inputControlEnabled: true });
  const sentEvents = [];
  ...
  const result = await bot.executeKeyboardTurn("west");
  const keyEvents = sentEvents.filter((event) => event.method === "Input.dispatchKeyEvent");

  assert.equal(result?.ok, true);
  assert.equal(result?.transport, "keyboard-turn");
  assert.equal(result?.key, "ArrowLeft");
  assert.equal(result?.direction, "west");
  assert.equal(keyEvents.length, 4);
  assert.deepEqual(keyEvents.map((event) => event.params.type), ["rawKeyDown", "rawKeyDown", "keyUp", "keyUp"]);
  assert.deepEqual(keyEvents.map((event) => event.params.key), ["Control", "ArrowLeft", "ArrowLeft", "Control"]);
  assert.deepEqual(keyEvents.map((event) => event.params.modifiers), [2, 2, 2, 0]);
});
...
test("followTrainWalk uses keyboard before viewport fallback when stuck on a field", async () => {
  const bot = new MinibiaTargetBot({
    partyFollowEnabled: true,
    partyFollowMembers: ["Knight Alpha", "Scout Beta"],
  });
...
test("buildFollowTrainFloorTransitionAction walks onto the exact stair tile across desynced floors", () => {
  const bot = new MinibiaTargetBot({
    partyFollowEnabled: true,
    partyFollowMembers: ["Czarnobrat", "Zlocimir Wielkoportf"],
  });
  bot.getNow = () => 10_000;
  bot.followTrainState = {
    leaderName: "Czarnobrat",
    lastSeenAt: 9_900,
  };

  const action = bot.buildFollowTrainFloorTransitionAction(
    createModuleSnapshot({
      playerName: "Zlocimir Wielkoportf",
      playerPosition: { x: 32823, y: 32144, z: 4 },
      reachableTiles: [
        { x: 32822, y: 32143, z: 4 },
        { x: 32822, y: 32144, z: 4 },
        { x: 32821, y: 32144, z: 4 },
  ...
  assert.equal(action?.kind, "follow-train-walk");
  assert.equal(action?.reason, "floor-change");
  assert.deepEqual(action?.destination, { x: 32821, y: 32144, z: 4 });
  assert.equal(action?.progressKey, "32821,32144,4");
});
